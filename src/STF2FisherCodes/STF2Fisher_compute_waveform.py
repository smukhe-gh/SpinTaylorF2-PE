...
def to_lal_coords(m1, m2, chi1, kappa, thetaJ, psiJ, alpha0, f_inj):
    ...
    shat = rotateY(shat, -incl) # Additional rotation [Not in initial code written by Haris]
    spin = np.array(shat) 		
...
class waveform:
	...
	def __init__(self, f_inj, f_max, delta_f, amplitude_order=0, phase_order=7, spin_order=6, approximant='SpinTaylorF2', sideband=None, **kwargs):
		self._finj       = f_inj
...
class fisher:
	...
	def _deriv(self, wf_params, key, dval):
		pcpy       = wf_params.copy()
		pcpy[key] += dval
		h2         = self._wfgen.waveform(**pcpy) #[Check what waveform is being generated here.]
		pcpy       = wf_params.copy()
		pcpy[key] -= dval
	...
	def _calc_derivs(self, wf_params, wf_derivs, deriv_lst):
		h0     = self._wfgen.waveform(**wf_params) #[Check what waveform is being generated here.]
