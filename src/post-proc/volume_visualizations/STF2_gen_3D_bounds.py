...
for k, file in enumerate(files):
	if not (k+1)%2 == 0:	
		...
		if select_plot == "KAPPA":
			# ax1.plot(XF, BOUNDS[0], alpha = 0.8, linewidth=0.8, color=COLORS[k])
			# ax1.plot(XF, BOUNDS[1], alpha = 0.8, linewidth=0.8, color=COLORS[k], label=r'$\eta=%1.2f$'%DATA["ETA"])
			...
			# ax1.plot(XF, BOUNDS[0], alpha = 0.8, linewidth=1.2, color=COLORS[k+1])
			# ax1.plot(XF, BOUNDS[1], alpha = 0.8, linewidth=1.2, color=COLORS[k+1], label=r'$\eta=%1.2f$'%DATA["ETA"])
			ax1.plot(XR, BR0, alpha = 0.8, linewidth=1.2, color=COLORS[k+1])
			ax1.plot(XR, BR1, alpha = 0.8, linewidth=1.2, color=COLORS[k+1], label=r'$\eta=%1.2f$'%DATA["ETA"])
			ax1.fill_between(XF, BOUNDS[0], BOUNDS[1], facecolor=COLORS[k+1], alpha=0.1)
...
if select_plot == "KAPPA":
	...
	ax1.set_ylabel(r'$\kappa_{\rm UB}$')
	ax1.set_title(r'Upper bound on $\kappa$')
	# ax1.savefig("./test_kappa.pdf")
elif select_plot == "CHI":
	ax1.axvline([0], linewidth=4, color='white')
	ax1.set_xlim(0, 1.5)
...
for k, file in enumerate(files):
	if not (k+1)%2 == 0:	
		...
		if select_plot == "KAPPA":
			...
			# ax2.plot(XF, BOUNDS[0], alpha = 0.8, linewidth=1.2, color=COLORS[k+1])
			# ax2.plot(XF, BOUNDS[1], alpha = 0.8, linewidth=1.2, color=COLORS[k+1], label=r'$\eta=%1.2f$'%DATA["ETA"])
			ax2.plot(XR, BR0, alpha = 0.8, linewidth=1.2, color=COLORS[k+1])
			ax2.plot(XR, BR1, alpha = 0.8, linewidth=1.2, color=COLORS[k+1], label=r'$\eta=%1.2f$'%DATA["ETA"])
			ax2.fill_between(XF, BOUNDS[0], BOUNDS[1], facecolor=COLORS[k+1], alpha=0.1)
...
if select_plot == "KAPPA":
	...
	# ax2.savefig("./test_kappa.pdf")
elif select_plot == "CHI":
	ax2.axvline([0], linewidth=4, color='white')
	ax2.set_xlim(0, 1.5)
	ax2.set_ylim(0.2, 0.8)
	ax2.set_xlabel(r'$H_{0}/H_{2}$')
	ax2.set_ylabel(r'$\chi_{\rm LB}$')
	ax2.set_title(r'Lower bound on $\chi_{1}$')
	# ax2.savefig("./test_chi.pdf")
