...
def _lalsim_td_waveform(**p):
    ...
    hp, hc = lalsimulation.SimInspiralChooseTDWaveform(float(p['coa_phase']),
               ...
               float(p['inclination']),
               float(p['lambda1']),  float(p['lambda2']), flags, 
               None,
...
def _lalsim_fd_waveform(**p):
    ...
    lalsimulation.SimInspiralSetSpinOrder(flags, p['spin_order'])
    lalsimulation.SimInspiralSetTidalOrder(flags, p['tidal_order'])
    #added to pull out harmonics: SM
    nonGRparams = lalsimulation.SimInspiralCreateTestGRParam('sideband',p['sideband'])
    ...
    hp, hc = lalsimulation.SimInspiralChooseFDWaveform(float(p['coa_phase']),
        ...
        pnutils.megaparsecs_to_meters(float(p['distance'])),
        float(p['inclination']),
        float(p['lambda1']), float(p['lambda2']), 
        flags, 
        nonGRparams,
        amplitudeO=int(p['amplitude_order']), 
        phaseO=int(p['phase_order']),
        approximant=_lalsim_enum[p['approximant']])
